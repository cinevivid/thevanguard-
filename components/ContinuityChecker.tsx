...
const ContinuityChecker: React.FC = () => {
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    setAnalysis('');
    try {
      const stream = analyzeScript('continuity_errors');
      let fullResponse = '';
  ...
  return (
    <div className="space-y-8">
      <h1 className="text-3xl font-bold">AI Continuity Checker</h1>
      <p className="text-vanguard-text-secondary">Let the AI act as a script supervisor, scanning the entire screenplay for potential continuity errors in plot, character, and props.</p>
      
      <Card title="Scan for Errors">
        <button onClick={handleAnalyze} disabled={isLoading} className="bg-vanguard-accent hover:bg-vanguard-accent-hover text-white font-bold py-2 px-4 rounded-lg mb-4">
          {isLoading ? 'Scanning...' : `Check for Continuity Errors`}
        </button>
        <div className="mt-4 p-4 bg-vanguard-bg rounded-md min-h-[200px] max-h-[60vh] overflow-y-auto">
          {isLoading && !analysis && <p>AI is reading the script for potential inconsistencies...</p>}
