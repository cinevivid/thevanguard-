...
const PacingRhythm: React.FC = () => {
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    setAnalysis('');
    try {
      const stream = analyzeScript('pacing_rhythm');
      let fullResponse = '';
  ...
  return (
    <div className="space-y-8">
      <h1 className="text-3xl font-bold">Pacing & Rhythm</h1>
      <p className="text-vanguard-text-secondary">Analyze the screenplay's tempo, identifying high-action sequences versus moments of quiet introspection to map the overall rhythm of the film.</p>
      
      <Card title="Pacing Analysis">
        <button onClick={handleAnalyze} disabled={isLoading} className="bg-vanguard-accent hover:bg-vanguard-accent-hover text-white font-bold py-2 px-4 rounded-lg mb-4">
          {isLoading ? 'Analyzing...' : `Analyze Pacing & Rhythm`}
        </button>
        <div className="mt-4 p-4 bg-vanguard-bg rounded-md min-h-[200px] max-h-[60vh] overflow-y-auto">
          {isLoading && !analysis && <p>AI is analyzing the screenplay's pacing...</p>}
